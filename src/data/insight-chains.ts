...
const AI_INFRA: InsightChain = {
  ...
  disclaimer:
    "本内容为 AI 推理基础设施产业链的关联梳理,基于公开、公认的行业因果逻辑。当前页面包含部分已核实官方来源和部分常设核验入口;演示事件仍为样板事件,正式上线时会替换为当天真实事件,并为每一跳补充具体来源、发布时间和引用字段。文中所有关系均为「关联/映射/受影响」,属非确认因果关系、仅统计非预测,不构成任何投资建议、不含买卖指令、目标价或涨跌预测。多跳链为「逻辑 + 证据」框架,非回测统计胜率。本工具不提供证券投资咨询服务,所列个股仅为产业链关联的说明性示例,不构成任何推荐;个股波动风险自负。",
};
