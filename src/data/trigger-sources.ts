...
const AI_BROAD = /AI|人工智能|大模型|生成式|Copilot|Agent|智能体|LLM|GPT/i;
const AI_COMMERCIAL =
  /AI\s*(收入|订单|营收|付费|订阅|商业化|revenue|subscription|ARR|adoption|workload)|企业级\s*AI|enterprise\s*AI|Copilot|AI\s*agent|智能体|客户采用|customer\s*adoption|商业化落地|AI\s*(产品|业务)/i;
...
export function routeInsightForItem(item: {
  ...
  if (!src) return null;
  const text = `${item.title} ${item.retailTake ?? ""}`;
