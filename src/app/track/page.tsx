...
import { listOutcomes, summarize, HIT_THRESHOLD } from "@/lib/outcomes";
...
export default async function TrackPage() {
  const rows = await listOutcomes(300).catch(() => []);
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] text-[#1a1d24]">
      ...
      <main className="mx-auto max-w-3xl px-4 py-6 sm:px-6">
        ...
        {/* 规则透明 */}
        <div className="mb-4 rounded-lg bg-gray-100 px-4 py-3 text-xs leading-relaxed text-gray-500">
        ...
        {rows.length === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-300 bg-white py-14 text-center">
            ...
            <div className="mt-1 text-xs text-gray-400">
              ...
              {storageBackend() !== "postgres" && "(当前未连数据库,记账暂不可用)"}
            </div>
          </div>
        ) : (
