...
// self-fetch 沿当前请求同源返回应用实例,避免公开域名规范化跳转把 POST 改成 GET;
...
export async function GET(req: NextRequest) {
  ...
  for (const fam of ["risk-events", "fin-checkup", "fund-flow"] as const) {
    if (!(await run(fam, `${base}/api/${fam}`, post({ codes }), 55_000))) break;
  }
  ...
  outer: for (const c of codes) {
    for (const fam of ["fundamentals", "similarity"] as const) {
      if (!(await run(`${fam}:${c}`, `${base}/api/${fam}?code=${c}`, { method: "GET" }, 20_000))) break outer;
