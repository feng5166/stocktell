...
// 变化列:↑ 转进场 / ↓ 转出场 / 变 其它档位切换 / 新 事件点名 / — 无变化
function changeCell(pair: SegIntentPair | undefined, named: boolean) {
  if (pair?.y && pair.t.intent !== pair.y.intent) {
    ...
    if (toBull)
      return { mark: "↑", cls: "text-emerald-600", text: `由${pair.y.label}转为${pair.t.label}` };
    if (toBear)
      return { mark: "↓", cls: "text-red-600", text: `由${pair.y.label}转为${pair.t.label}` };
    return { mark: "变", cls: "text-indigo-600", text: `由${pair.y.label}转为${pair.t.label}` };
  }
  if (named) return { mark: "新", cls: "text-brand-600", text: "今日被链级事件点名" };
  return { mark: "—", cls: "text-gray-300", text: "今天没新东西" };
}
...
export function HomeMyStocks({
  ...
  const rows = Array.from(wl.codes)
    .map((code) => {
      ...
      const active = chg.mark !== "—";
      return { code, name: s.name, info: watchChainMap[code], pair, chg, active };
    })
  ...
  return (
    <section className="mt-8">
      ...
      <div className="mt-3 overflow-hidden rounded-2xl bg-white shadow-sm">
        {/* 表头(桌面) */}
        <div className="hidden gap-3 border-b border-gray-100 px-4 py-2 text-meta text-gray-400 sm:grid sm:grid-cols-[1.1fr_1fr_5.5rem_2.5rem_1.4fr]">
          <span>股票</span>
          <span>所属逻辑</span>
          <span>资金意图</span>
          <span>变化</span>
        ...
        <div className="divide-y divide-gray-50">
          {rows.map((r) => (
            <Link
              key={r.code}
              href={`/stock/${r.code}`}
              className="block px-4 py-2.5 transition-colors hover:bg-gray-50 sm:grid sm:grid-cols-[1.1fr_1fr_5.5rem_2.5rem_1.4fr] sm:items-center sm:gap-3"
            >
              ...
              <span className="mt-0.5 block truncate text-xs text-gray-500 sm:mt-0">
                {r.info ? `${r.info.chainName} · ${r.info.segment}` : "—"}
              </span>
              ...
              <span
                className={`ml-2 inline-flex items-center text-sm font-semibold sm:ml-0 ${r.chg.cls}`}
                title={r.chg.text}
              >
                {r.chg.mark}
              </span>
              <span className="mt-0.5 block truncate text-xs text-gray-500 sm:mt-0">
                {r.active ? r.chg.text + (r.info?.verify[0] ? ` · ${r.info.verify[0]}` : "") : r.info?.verify[0] ?? "验证点照旧"}
              </span>
            </Link>
          ))}
        </div>
