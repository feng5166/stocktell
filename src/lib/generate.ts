...
function buildTake(m: Mover): string {
  ...
  if (m.change > 0) {
    const watch =
      pos === "上游"
        ? "开盘看订单/产能能不能兑现,分清是业务传导还是纯情绪"
        : pos === "下游"
        ? "开盘看需求侧有没有放量,情绪驱动的补涨往往缺持续性"
        : "开盘重点看能不能放量跟上,跟得上才是真共振、跟不上多半只是情绪";
    return `${lead}海外${m.name}${tag}${up},A股${names}${color}${watch};跟不上要么没轮到、要么本就不相关(海外营收占比低),先看量能验证。`;
  }
  const watchD =
    pos === "上游"
      ? "开盘看是订单预期变了还是纯情绪杀,是否「错杀」要看订单有没有变化"
      : pos === "下游"
...
- 也不要软性操作暗示:"别急着接/别往里冲/可逢回调关注/追高需谨慎/等回调再说/别重仓/控制仓位/上车"这类词都不要——它们本质还是在教用户怎么操作。
  → 用"观察/验证"口径替代:说清"这波是订单逻辑还是纯情绪映射""接下来要验证什么(订单/毛利率/客户/量能)",帮用户理解,不替用户操作。
