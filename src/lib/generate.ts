...
- 【硬性·违反即失败】绝不出现 A 股标的的具体涨跌数字,也不要写"今天涨/跌了X""涨了3个多点"这类表述:
  早报在盘前生成(A 股还没开盘),你拿到的 A 股涨跌是【上一交易日】的,写进正文会和页面顶部的【实时行情】打架(用户会发现对不上)。
  只用定性词(逆势走强 / 没怎么跟跌 / 跌得更狠 / 基本同步 / 相对抗跌),把"所以呢"讲透。美股隔夜涨跌幅是既成事实、可以提。
- 直接给"所以呢",别复述行情。
