    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          beneficiaryCodes: { type: "array", items: { type: "string" } },
          retailTake: { type: "string" },
        },
        required: [
          "impact",
          "title",
...
async function claudeDrafts(
  ...
  const parsed = JSON.parse(text) as { items: ClaudeItem[] };
...
export async function generateDrafts(): Promise<{
  ...
  if (useClaude && movers.length > 0) {
    try {
      drafts = await claudeDrafts(date, movers);
      engine = "claude";
    } catch {
