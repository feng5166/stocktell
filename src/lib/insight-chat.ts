...
const TRADING_INTENT = new RegExp(
  [
    // 买卖动作
    "买|卖|入手|上车|进场|离场|出货|接盘|出掉|该出|出手|抛|建仓|清仓|平仓", // copylint-allow(交易意图识别正则,护栏自身)
    // 仓位管理
    "加仓|减仓|补仓|换仓|调仓|满仓|空仓|梭哈|全仓|半仓|重仓|轻仓|几成仓|仓位|减持点|增持点", // copylint-allow(护栏正则)
    // 持有/退出族(review 点名)
    "持有|持仓|拿住|拿着|继续拿|还能拿|拿多久|割肉|解套|回本|落袋|获利了结|止损|止盈|止益", // copylint-allow(护栏正则)
    // 涨跌预测/空间(review 点名)
    "能涨|会涨|要涨|能跌|会跌|要跌|涨到|跌到|翻倍|涨停|跌停|上涨空间|下跌空间|涨幅空间|空间多大|还有多少空间|能到多少|涨多少|跌多少",
    // 价格/点位/择时
    "目标价|什么价位|多少钱可以|什么点位|买点|卖点|入场点|进出点|现在进|明天走势|后市怎么|择时|时机|等回调|抢反弹|博反弹", // copylint-allow(护栏正则)
    // 操作建议/风格(排除「操作系统」)
    "操作(?!系统)|怎么办|如何布局|怎么布局|值得买|值得入|能不能买|可以买|该买|该不该买|做T|打板|波段|短线|抄底|低吸|高抛|追高|追进", // copylint-allow(护栏正则)
    // 盈亏
    "挣钱|赚钱|赔钱|亏钱|能赚|会亏",
  ].join("|")
);
