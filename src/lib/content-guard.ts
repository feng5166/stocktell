...
const BANNED =
  /买入|卖出|建议买|建议卖|推荐买|抄底|满仓|加仓|减仓|清仓|低吸|接飞刀|站岗|目标价|建议参与|短线机会|回调关注|等回调|上车|别急着接|往里冲|追高|追涨|追板|低开|高开|企稳|放量|缩量|破位|补跌|超跌反弹|止跌|杀跌|洗盘|获利盘|兑现盘|错杀|出货|值得多看一眼|开盘盯|盘中盯|逢低|逢高|埋伏|潜伏|梭哈|梭一把|上车|下车|割肉|套牢/g;

// 产业语义白名单:先剥离这些合法指标用法,再扫禁词(如「出货量」「放量节奏」是产业指标,非盘面)
const INDUSTRIAL_WHITELIST =
  /出货量|放量节奏|订单放量|业务放量|产品放量|真正放量|批量出货|出货节奏|出货预期|出货占比/g;
