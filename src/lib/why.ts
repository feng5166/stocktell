...
type LLMResolved = NonNullable<Awaited<ReturnType<typeof getLLMFor>>>;
...
export async function explainMove(
  ...
  if (bochaEnabled()) {
    out = await retrievalWhy(llm, name, code, date, title); // 优先真实检索
  } else if (process.env.WHY_ENABLED) {
    out = await legacyWhy(llm, name, code); // 兼容旧开关
  } else {
