...
// 单次请求(受并发阀保护):仅在真正发起 fetch 时占槽位,结束即释放——
// 这样 tsCall 重试间的 1.5s 退避不会白占并发名额。
async function tsFetchOnce(
  apiName: string,
...
export async function longhuByDate(ymd: string): Promise<Map<string, LonghuHit>> {
  ...
  if (hit) return hit;
  return singleFlight(`lh:${ymd}`, async () => {
...
export async function marginByDate(ymd: string): Promise<Map<string, number>> {
  ...
  if (hit) return hit;
  return singleFlight(`mg:${ymd}`, async () => {
...
export async function dailyByDate(ymd: string): Promise<Map<string, number>> {
  ...
  if (hit) return hit;
  return singleFlight(`daily:${ymd}`, async () => {
    const cached = dCache.get(ymd);
    if (cached) return cached;
    const out = new Map<string, number>();
