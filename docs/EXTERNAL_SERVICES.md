...
| **Tushare** `api.tushare.pro` | 交易日历(trade_cal)/ 基本面(daily_basic)/ 资金面(moneyflow/top_list/margin)/ **AI 链内资金状态**与**股票池资金形态**(daily 成交额 + moneyflow 主力净额)/ **雷区雷达**(share_float 解禁、stk_holdertrade 增减持、pledge_stat 质押、repurchase 回购、namechange ST 判定)/ **财报体检**(income/balancesheet/cashflow/fina_indicator)/ **相似性**(dailyHistory 2年日线) | `src/lib/tushare.ts`、`segment-fund-status.ts`、`fund-behavior.ts`、`risk-radar.ts`、`financials.ts`、`similarity.ts`、`api/fundamentals`、`api/similarity` | 需 `TUSHARE_TOKEN`(6000积分)。AI 链内资金状态只聚合已核定链内成分,指标名为“链内净额/成交比”(主力净额合计 ÷ 同日成交额合计 × 100%),不代表全市场板块排名;结果落 `quotes_cache:segment-fund-status-v4`,源不完整不写缓存。股票池资金形态标签只使用同一收盘日的股价涨跌、净额/成交比和当日龙虎榜,不叠加历史窗口或融资余额变化;展开区另展示近 3 日逐日资金、融资变化与龙虎榜作为核验数据,不参与标签归类。数据完整时输出五类行为特征或偏强/弱势共振、资金分歧、震荡观望;只有同日数据缺失才输出“待判断”。所有标签最高中置信,不确认机构真实意图。其它基本面/资金面/雷区/财报/相似性结果落 **DB 跨实例缓存**——`quotes_cache`(按 `<类>:code:当天`)+ `fund_day_cache`(按 ymd)。资金日期必须先探测最新可用交易日,DB 旧日只作 last-good 回退;当日盘前成功返回的空 moneyflow 最多缓存 5 分钟,不得把新数据永久挡住。失败用 `tsCallStrict` 区分「回源失败 vs 成功但空行」:**只在确有回源时才写缓存(不毒化)** + `alertThrottled` 飞书告警。资金整包已由 `fund_day_cache` 按 ymd 复用,调用层不再叠加缺少交易日维度的 `unstable_cache`。 |
