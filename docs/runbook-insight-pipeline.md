...
## 同图谱检测(§7.2-6,防退化成预制图谱)
- 每次生成算「热力方向指纹」(环节:方向 排序拼接),与最近 2 天比。
- 连续 2 天完全一致 → 审核卡片标 ⚠️ 警告,提醒人审重点看。
- 连续 3 天完全一致 → **管线自动暂停** + 升级飞书告警;当日草稿仍落库供人审判断,但次日起 cron 跳过,直到 `force=1` 恢复。
- ⚠️ 这是**启发式**:真实连续行情(连跌数日)也会命中,不是 bug——人工确认即可,`force` 恢复。
