...
| 看门狗 | 08:30 心跳升级:除"简报存在"外,**核对当天 `digest_send_log` 记录数**(每成功发一个用户写一条,当"邮件确实发过"的证据)。简报在而记录 0 条 → ❌ 告警"疑似未发,请补推"而非 ✅ |
...
## 8. 同日事故 #2:早报缓存跨日污染(「Meta 涨」说了一整天)

**现象**(用户 07-03 上午发现):今日简报全是跌(泛林 -10.19%、迈威尔 -9.84%…,且当日无 Meta 条目),但「你的今日早报」说"Meta大涨是利好"——方向整个反了;07:01/08:03 的盘前邮件带的也是这段错文案。

**根因**:`getMorningBrief` 缓存 key 此前 = `v3:{todayISO()}:{自选hash}`。00:00~07:01 之间今日简报还没生成,首页回退展示**昨日**条目(带"以下为最近一期"横幅),此时访问首页会用昨日条目生成早报、**缓存到今天的 key**。07:01 简报换新后,网页和邮件全部命中这条被污染的缓存。Meta +8.81% 是 07-02 简报的内容。

**取证**(`GET /api/admin/brief-cache?date=2026-07-03`,新增只读排障端点):当日 10 条缓存,2 条写入于北京 00:10:07 / 00:21:34(凌晨访问,内容=07-02 叙事,含"Meta大涨"),8 条写入于 07:01(cron,内容=07-03 跌势叙事)——时间戳与内容方向一一对应,实锤。

**修复**(同日上线):
- key 改用**条目自身的 date**(`v4:{items[0].date}:{hash}`):凌晨用昨日条目 → 落昨日 key(通常直接命中昨天已有缓存,零 LLM 成本,且与页面回退展示口径一致);07:01 邮件/网页用今日条目 → 今日 key,互不污染。条目日期只允许 ≤ 今天,挡伪造未来日期预写缓存。
- 版本 v3→v4:作废已污染的当天缓存,用户下次访问自愈重生成。
- 前端:回退期早报标题不再自称"今日",改「☀️ 你的早报(最近一期 · MM-DD)」。
- 新增 `GET /api/admin/brief-cache?date=&full=1`:查某日早报缓存生成时间与正文,这类"早报内容陈旧/方向不对"问题直接看时间戳定位。

## 9. 同日追加:v4 修复的三轮评审加固(缓存投毒/烧钱洞根治)

对 §8 的 v4 修复做了三轮多智能体对抗评审(每轮 15 条确认发现,逐轮修复收敛),核心结论:**v4 只修了"意外污染",没堵住"人为投毒"**——`POST /api/morning-brief` 无鉴权且信任客户端传的 items,任何人可预写共享缓存(内容投毒进邮件/网页)、用垃圾日期或 codes 组合无限刷新 key 白烧 LLM。最终形态(同日上线):

| 层 | 加固 |
|---|---|
| 数据源 | 条目一律服务端解析(`resolveMorningItems`,与首页回退口径一致):`/api/morning-brief`、早报深读、资金面深读(改 `fundFlowFor` 取真)全部忽略客户端 items |
| 缓存 key | **v5 归一化** = 条目 id + 与条目相关的自选码(无关码不进 key、资金面段也只讲相关票);深读升版 `morningv2:`/`fundflowv2:`(+条目 id),历史可疑行从读路径失联 |
| 花费保险丝 | 早报每期生成 300 条软顶(超限走模板不缓存;`trusted` 直通给 digest/微信,匿名刷满不影响真实用户邮件);深读每期 150 条(超限 503 可重试) |
| hint | 前端带"正在展示那期"的日期让卡片与 ISR 页面同期,但服务端只认{今天,最近一期}——放开任意历史期=熔断预算×历史期数的乘法器 |
| 输入卫生 | STOCK_MAP 池校验用 hasOwnProperty(挡 "constructor" 等原型链属性名);JSON null body 不再 500;date 一律严格 YYYY-MM-DD |
| 重刷一致性 | `generate?replace=1` 作废当日派生缓存(v5/v4/早报深读)+ 重生成 ai 链级判断 + 链后二次清扫(防在途 LLM 写回旧叙事);purged 如实上报;maxDuration 60→300 |
| 清理 | watchdog 每交易日心跳后清扫:早报缓存 14 天 TTL;深读 30 天 TTL(仅按日 key 家族,条目 id 键的历史解读存档不删,防翻旧简报时按今日行情错乱重生成);老前缀 morning:/fundflow: 即清 |
| UI 口径 | 早报卡标题绑定接口返回的 date/stale(与正文永远同期,不用设备时钟);模板兜底不再自称"今天";Quiet 卡回退期措辞不许诺"今天会有新一期"(节假日成立);DeepRead 404=终态原文展示、503=可重试 |
| 回归测试 | smoke.sh:相关码必出早报(防解析回归静默全灭)、投毒标记词不得出现在正文、null body、原型链属性名 |

**接受的残留(评审确认、有意取舍)**:①早报正文的资金面段收窄到"与当日叙事相关的票"(v5 共享 key 的正确性要求;其余持仓的资金/雷区仍由邮件 ⚠️ 告警区全覆盖)②熔断是先查后生成的软顶,并发窗口可少量超发(花费保险丝够用,不为它上原子计数器)③DB 瞬时抖动时早报卡整块隐藏(诚实沉默,优于错误口径)④Quiet 卡的陈旧判断仍用设备时钟(无 API 可依,影响极低)。
