...
## 优化进展 / 复测对比
| 项 | 优化前(香港·坏窗口) | 优化后(东京 hnd1) | 状态 |
|---|---|---|---|
| **行情 quotes** | ~5.9s(每次,新浪封 IP 拖 5s) | **0.75–1.14s** | ✅ 腾讯优先(`6871f86`)+ 区域东京 |
| **基本面 fund** | ~14s 间歇(超前端 12s→不显示) | **0.75–1.25s** | ✅ 区域东京(`28fb124`) |
| **联动 similar** | ~1–2s | 0.74–1.77s | ✅ |
| **HTML 壳** | 冷 ~9s | 0.88–2.14s | ✅ |

**根因锁定(三区同窗口 15 轮实测)**:病根是**香港区(hkg1)到阿里云北京(Tushare/博查)那条路又慢又抖**(300–600ms + 间歇 connect-timeout)。东京(hnd1)/新加坡(sin1)到阿里云北京都是干净的 **~65ms、15/15 稳**。→ **锁定东京 hnd1**(`vercel.json regions:["hnd1"]`),病根绕开。东京对阿里云北京快且稳,又是"好区里离大陆最近的"。
**决定**:**不走 VPS 中继、不需要 DB 连接串**——一次区域切换全解决。(SG VPS 实测:→Tushare 稳定 ~500ms/10-of-10,本可当中继,但区域方案更省,故不用;VPS 保留跑 ilink/umami。)
**残留(已两轮复测确认属罕见)**:Tushare 偶发抽风时,similarity/fundamentals 仍可能被 app 的 6s×2 超时放大成 ~14s(2 轮共 ~40 个 similarity 样本里出现 1 次)。治标:收紧 Tushare 单次超时(6s→3–4s)+ 少/快重试,把最坏情况从 14s 降到 ~4–8s。属可选打磨,非系统性。
