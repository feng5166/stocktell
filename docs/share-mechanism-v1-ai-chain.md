...
## 附:2026-07-03 链页评审拍板(负责人)

**结论:这版 `/chain/ai` 作为分享 V1 落地页合格(8/10),可开始小流量验证分享→点击→加自选/订阅漏斗;但作为"产业链页"还不合格(5.8/10)——板块分组的股票清单+单跳动态,缺链级判断/传导路径/环节热力/insight 深读。可跑分享验证,不宜大规模铺量;铺量前至少补上链级判断、insight 深读出口和动态文案。**

P0(当日已上线,不动分享骨架):
1. 顶部「今天怎么看这条链」链级一句话判断:为什么强/弱、传导最直接的环节、哪些只是情绪映射。07:01 cron 生成(lib/chain-take.ts,KV=chaintake:{date}:{chain}:t1),链页只读缓存保 ISR,缺失用规则兜底;admin 重跑:POST /api/admin/chain-take?force=1。
2. 关键动态每条挂「看它怎么传到 A 股 →」深读出口 → /insight/{chain.insightSlug}(chains.ts 新增 insightSlug,ai→ai-infra);链级判断卡也带「看完整因果链」。
3. 成分股清单动态化第一步:今天被简报点名的票行内标「今日被提到 · {条目标题}」并在组内置顶(复用当日 items,零额外成本)。检索式 per-stock why 因成本(80 只×搜索)不上 roster,留在个股页。

P0.5(当日已上线):
- 交易化语言下调:静态 take 28 处(追高/追板/逢回调/别急着接/别重仓赌…)改"观察/验证/情绪映射 vs 订单兑现"口径;generate.ts 与 gen-retail-takes.mjs 提示词加软性操作暗示禁语(明日动态文案起效)。原则:**帮你理解,不替你操作**。
- 订阅文案:"每天早上…一眼看懂/订阅每日解读" → "订阅{链}每日推理:隔夜事件怎么传到这条链、哪些票只是情绪映射"。

P1(下一批):「这条链有哪些票」从板块分组升级为**链环节分组 + 每环节今日温度**(温度/为什么/相关股票三字段),外加极简传导箭头(隔夜事件→环节→A股映射)。页面从"股票清单"变"产业链热力"。
